...
var DRIFT_NAMES = ["⬆ UP", "⬇ DOWN", "⬅ LEFT", "➡ RIGHT"];
var DRIFT_DIRS = [[0, -1], [0, 1], [-1, 0], [1, 0]];
  ...
  handleInput(px, py) {
    ...
    if (this.phase === "recall") {
      ...
      if (gx < 0 || gx >= GRID_SIZE || gy < 0 || gy >= GRID_SIZE)
        return;
      if (this.selectedCells.some(([x, y]) => x === gx && y === gy))
        return;
      this.selectedCells.push([gx, gy]);
      const drifted = this.activeCells.map(([x, y]) => [
